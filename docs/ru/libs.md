...
local kb = pagination({
  ...
  command = 'cb_list',                 -- зарегистрированная callback-команда (см. Команды)
  arguments = { action = 'page' },     -- nav-кнопки наследуют это; page ставится на каждой
...
Построено на `inlineCallbackKeyboard`: навигационные кнопки и кнопки элементов
кодируют `callback_data` из `arguments_schema` callback-команды. См. пример
